...
export class PaymentComponent implements OnInit, OnDestroy {
	...
	paymentSessionTimeout = 10 * 60; // 15 minutes in seconds
	...
	ngOnInit() {
		...
		// Start 15-minute session timeout for payment
	...
	submitCustomerForm() {
		if (this.sessionExpired) {
			return; // Prevent actions if session expired
		}
				...
				.subscribe({
					next: (response: ClientDebitSuccessDTO | ErrorResponse) => {
						...
						if (successResponse.statusCode === '00') {
							...
							// Clear the payment session timer since payment is successful
	...
	/**
	 * Start 15-minute payment session timer
	 */
	private startPaymentSessionTimer(): void {
		...
		console.log(
			`Starting payment session timer: ${this.paymentSessionTimeout} seconds`
		);
	...
	private handlePaymentSessionTimeout(): void {
		...
		console.log('Payment session timeout triggered');
		...
		this.confirmationService.confirm({
			message:
				'Your payment session has expired. You will be redirected to start a new booking.',
			...
			accept: () => {
				// Close the payment dialog and navigate to home
				this.ref.close({ success: false, sessionExpired: true });
				this.router.navigate(['/']);
			},
		});
	}
	...
	/**
	 * Reset the payment session timer (call this on successful payment)
	 */
