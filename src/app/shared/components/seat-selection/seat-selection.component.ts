...
export class SeatSelectionComponent implements OnInit, OnDestroy {
	...
	@Input() config: SeatSelectionConfig = {
		maxSeats: 10,
	...
	sessionTimeout: number = 900; // 15 minutes default
	...
	loadSeatData(): void {
		...
		const requests = [
			...
			this.bookingService.getOccupiedSeatsBySession(
				this.screening.id,
				this.sessionId || ''
			),
		];
		...
		forkJoin(requests)
			...
			.subscribe({
				next: ([seatsResponse, occupiedSeatResponse]) => {
					...
					this.hall = this.seats.length > 0 ? this.seats[0].hall || null : null;
					this.seatCategories = this.getUniqueSeatCategories(this.seats);

					// Initialize seat availability
					this.initializeSeatAvailability(
						this.seats,
						occupiedSeatResponse as OccupiedSeatResponse
					);
			...
			this.sessionSeats = occupiedSeatResponse.sessionSeats.map(
				(seat) =>
					({
						...
						expiresAt: new Date(
							Date.now() + this.sessionTimeout * 1000
						).toISOString(),
						userMetadata: {
	...
	private refreshOccupiedSeats(): void {
		...
		this.bookingService
			.getOccupiedSeatsBySession(this.screening.id, this.sessionId)
			.pipe(takeUntil(this.destroy$))
			.subscribe({
				next: (response) => {
				...
				next: (response) => {
					if (response.success) {
						...
						if (response.occupiedSeats) {
							const occupiedSeatResponse: OccupiedSeatResponse = {
	...
	getUniqueSeatCategories(seats: Seat[]): SeatCategory[] {
		const categories: SeatCategory[] = [];
		const categoryIds = new Set<number>();

		seats.forEach((seat) => {
			if (seat.category && !categoryIds.has(seat.category.id)) {
				categories.push(seat.category);
				categoryIds.add(seat.category.id);
			}
		});
